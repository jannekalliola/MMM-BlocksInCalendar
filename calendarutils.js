...
const CalendarUtils = {
	...
	filterEvents: function (data, config) {
		...
		Object.entries(data).forEach(([key, event]) => {
			...
			const future = moment().startOf("day").add(config.showWeeks, "weeks").subtract(1, "seconds").toDate(); // Subtract 1 second so that events that start on the middle of the night will not repeat.
			let past = today;
				...
				if (typeof event.rrule !== "undefined" && event.rrule !== null && !isFacebookBirthday) {
					...
					for (let d in dates) {
						...
						if (showRecurrence === true) {
							let dayDuration = 0;
							if(CalendarUtils.isFullDayEvent(event)) {
								dayDuration = endDate.diff(startDate, "days");
							}
							else {
								dayDuration = moment(endDate).add(1, "days").startOf("day").diff(moment(startDate).startOf('day'), "days");
							}
							if(dayDuration < config.minimumEntryLength) {
								return;
							}
